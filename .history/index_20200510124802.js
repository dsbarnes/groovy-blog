...
const handleRemoveData = (currentSection) => {
    const section = Array.from(document.querySelectorAll('[id*="section"]'))
    // if (section) for (const element of section) element.remove()
    section.filter(container => container.id != `${currentSection}-section-text`)

    for (const container of section)
